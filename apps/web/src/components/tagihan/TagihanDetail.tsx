...
import { useTagihanDetail } from '@/features/tagihan/hooks/useTagihanDetail';
import type { Struk, Tagihan } from '@/lib/types';
...
export default function TagihanDetail({ item, onBack, onRefresh, onEdit, onDelete }: Props) {
  ...
  const { strukList, history, payMut, revertMut, uploadMut, deleteStrukMut } = useTagihanDetail(item, onRefresh);
  ...
  const submitPayment = () => {
    payMut.mutate({ amount: payAmount, date: payDate, note: payNote }, {
      onSuccess: () => {
        setPayOpen(false);
        setPayAmount(0);
        setPayNote('');
  ...
  const confirmRevertPayment = () => {
    ...
    revertMut.mutate({ historyId: revertTarget.id, jumlah: revertTarget.jumlah }, {
      onSuccess: () => {
  ...
  const uploadStruk = (file: File) => {
    uploadMut.mutate({ file, keterangan: strukKet }, {
      onSuccess: () => {
  ...
  const deleteStruk = (id: string) => {
    deleteStrukMut.mutate(id, {
      onSuccess: () => toast({ title: 'Struk dihapus' }),
    });
  };
    ...
    <div ref={ref}>
      ...
      <Dialog open={payOpen} onOpenChange={setPayOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <form onSubmit={e => { e.preventDefault(); if (validateQuickPay(item, payAmount).valid) submitPayment(); }} className="space-y-4 mt-2">
          ...
          {revertTarget && (
            <div className="space-y-4 mt-2">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                ...
                <button onClick={confirmRevertPayment} disabled={revertMut.isPending} className="px-4 py-2.5 rounded-lg text-sm font-medium bg-destructive text-destructive-foreground hover:opacity-90 transition-all disabled:opacity-50">
