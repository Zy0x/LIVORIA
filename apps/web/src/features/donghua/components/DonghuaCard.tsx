...
export function extractAltTitles(item: DonghuaItem) {
  return extractMediaAltTitles(item);
}

export function extractExtra(item: DonghuaItem): AnimeExtraData {
  return extractMediaExtra(item);
}

export function getWatchStatus(item: DonghuaItem): WatchStatus {
  return getDonghuaWatchStatus(item);
}

export function FilmBadge({ size = 'sm' }: { size?: 'xs' | 'sm' }) {
...
export const WatchlistCard = memo(function WatchlistCard({ item, onUpdateWatchStatus, onUpdateEpisode, onEdit, onDelete, onView, titleLang = 'original' }: WatchlistCardProps) {
  ...
  return (
    <div
      className={`donghua-watchlist-card group relative rounded-2xl border overflow-hidden cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${wsCfg.bg}`}
      onClick={onView}
    >
      {/* Watch status ribbon */}
...
export const DonghuaCard = memo(function DonghuaCard({
...
export function AddCard({ viewMode, onClick }: { viewMode: ViewMode; onClick: () => void }) {
  return <MediaAddCard viewMode={viewMode} onClick={onClick} trigger="donghua" listLabel="Tambah Donghua / Film Baru" />;
}
