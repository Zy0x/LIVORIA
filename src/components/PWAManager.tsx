 ...
 * 
 * Modern, estetik PWA install notification untuk LIVORIA.
 * Support: Chrome/Edge (beforeinstallprompt), iOS Safari (manual steps),
 * dan update notification.
 */

import { useState, useEffect, useRef } from 'react';
import gsap from 'gsap';
import { Shield, Download, X, RefreshCw, Wifi, WifiOff, Share, Plus, Smartphone } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[200] flex items-end justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div ref={modalRef} className="w-full max-w-sm bg-card rounded-3xl border border-border shadow-2xl overflow-hidden">
        ...
        <div className="relative px-5 pt-5 pb-4 bg-gradient-to-br from-primary/10 to-transparent border-b border-border/50">
          <button onClick={close} className="absolute top-4 right-4 p-1.5 rounded-full bg-muted/60 hover:bg-muted transition-colors">
        ...
        <div className="px-5 py-4 space-y-3">
          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Langkah-langkah:</p>
...
// ─── Install Banner ────────────────────────────────────────────────────────────
function InstallBanner({ onInstall, onDismiss, isIOS, onShowIOSModal }: {
  onInstall: () => void;
  onDismiss: () => void;
  isIOS: boolean;
  onShowIOSModal: () => void;
  ...
  useEffect(() => {
    if (bannerRef.current) {
      gsap.fromTo(bannerRef.current,
        ...
        { opacity: 1, y: 0, scale: 1, duration: 0.55, ease: 'back.out(1.4)', delay: 1.5 }
      );
    }
  }, []);
      ...
      <div className="relative bg-card border border-border/80 rounded-3xl shadow-2xl shadow-black/20 overflow-hidden">
        ...
        <div className="p-4">
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0 pt-0.5">
              <div className="flex items-center justify-between">
                ...
                <button onClick={dismiss} className="p-1 rounded-full hover:bg-muted/60 transition-colors ml-1">
                  <X className="w-3.5 h-3.5 text-muted-foreground" />
                </button>
              </div>
          ...
          <div className="flex gap-2">
            <button
              onClick={isIOS ? onShowIOSModal : onInstall}
    ...
    <div ref={bannerRef} className="fixed top-4 left-4 right-4 z-[100] max-w-sm mx-auto">
      <div className="bg-card border border-border rounded-2xl shadow-xl px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground">LIVORIA versi terbaru siap digunakan</p>
        </div>
...
export default function PWAManager() {
  ...
  // Show update banner when update available
  useEffect(() => {
  ...
  return (
    <>
      ...
      {pwa.canInstall && !pwa.isStandalone && (
        <InstallBanner
          onInstall={pwa.installPrompt}
          onDismiss={pwa.dismissInstall}
          isIOS={pwa.isIOS}
          onShowIOSModal={() => setShowIOSModal(true)}
        />
      )}
