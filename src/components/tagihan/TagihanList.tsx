...
function getDateLabel(t: Tagihan): string {
  ...
  if (t.tanggal_jatuh_tempo) {
    return new Date(t.tanggal_jatuh_tempo).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
  }
  return '—';
}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="tagihan-table text-left px-5 py-3.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap">
                  Debitur &amp; Barang
                </th>
                <th className="tagihan-table text-right px-4 py-3.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap">
                  Modal
                </th>
                <th className="tagihan-table text-right px-4 py-3.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap">
                  Cicilan/Bln
                </th>
                <th className="tagihan-table text-center px-4 py-3.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap">
                  Progress
                </th>
                <th className="tagihan-table text-right px-4 py-3.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap">
                  Sisa
                ...
                <th className="tagihan-table text-center px-4 py-3.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap w-28">
                  Aksi
                </th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-5 py-4">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 leading-tight line-clamp-1">
                        {t.barang_nama}
                        {t.debitur_kontak && <span className="text-muted-foreground/60 ml-1.5">· {t.debitur_kontak}</span>}
                      </p>
            ...
            <div key={t.id} className="rounded-2xl border border-border bg-card overflow-hidden">
              <button onClick={() => onView(t)} className="w-full text-left p-4 hover:bg-muted/20 transition-colors">
                ...
                <div className="mt-3 space-y-1.5">
                  <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                    <span className="flex items-center gap-0.5 text-success font-medium">
                      <CheckCircle2 className="w-2.5 h-2.5" /> {paid.count}x dibayar
                    </span>
                    <span>{pct.toFixed(0)}% · {fmtShort(Number(t.cicilan_per_bulan))}/bln</span>
                  </div>
                  <ProgressBar value={Number(t.total_dibayar)} total={Number(t.total_hutang)} />
                </div>
              </button>
