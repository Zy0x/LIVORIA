...
import {
  ...
  BarChart, Bar, CartesianGrid, AreaChart, Area, PieChart, Pie, Cell, RadialBarChart, RadialBar
} from 'recharts';
import {
  TrendingUp, TrendingDown, Users, Banknote, BarChart3,
  CalendarCheck, AlertCircle, CheckCircle2, Clock, Wallet,
  ChevronRight, ArrowUpRight, Activity, Zap, Target, Award,
  CreditCard, AlertTriangle, Circle
} from 'lucide-react';
...
const STATUS_COLORS: Record<string, string> = {
  aktif: CHART_COLORS.info,
  lunas: CHART_COLORS.success,
  overdue: CHART_COLORS.destructive,
  ditunda: CHART_COLORS.warning,
};
...
export default function TagihanLaporan({ data, onView }: Props) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo('.laporan-hero', { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out' });
      gsap.fromTo('.laporan-kpi', { opacity: 0, y: 30, scale: 0.95 }, {
        opacity: 1, y: 0, scale: 1, stagger: 0.07, duration: 0.5, ease: 'back.out(1.4)', delay: 0.2
      });
      gsap.fromTo('.laporan-section', { opacity: 0, y: 20 }, {
  ...
  const monthlyTrend = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      data.forEach(t => {
        ...
        if (start.getFullYear() === yr && start.getMonth() === mo) {
          modal += Number(t.harga_awal);
        }
      });
      months.push({ name, keuntungan: Math.round(keuntungan), cicilan: Math.round(cicilan), modal: Math.round(modal) });
    }
    return months;
  }, [data]);
  ...
  const cashflow = useMemo(() => {
    return Array.from({ length: 6 }, (_, i) => {
      ...
      data.forEach(t => {
        ...
        const start = new Date(t.tanggal_mulai);
        const endDate = new Date(start.getFullYear(), start.getMonth() + t.jangka_waktu_bulan, 0);
        if (d >= start && d <= endDate) { masuk += Number(t.cicilan_per_bulan); count++; }
      });
      return { name, masuk: Math.round(masuk), tagihan: count };
    });
  }, [data]);
  ...
  const topDebtors = useMemo(() => {
    ...
    data.filter(t => t.status !== 'lunas').forEach(t => {
      map.set(t.debitur_nama, (map.get(t.debitur_nama) || 0) + Number(t.sisa_hutang));
    });
  ...
  const kpis = [
    ...
    },
    {
      ...
      sub: `${data.filter(t => t.status !== 'lunas').length} aktif`,
      icon: Banknote,
      color: 'hsl(var(--info))',
      bg: 'bg-info/8',
    ...
    },
    {
      ...
      bg: collectRate >= 50 ? 'bg-success/8' : 'bg-warning/8',
      border: collectRate >= 50 ? 'border-success/15' : 'border-warning/15',
      trend: collectRate >= 50 ? 'up' : 'down',
    },
  ];
  ...
  return (
    <div ref={ref} className="space-y-5 sm:space-y-6">
      ...
      <div className="laporan-hero relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-card via-card to-primary/5 p-5 sm:p-6">
        ...
        <div className="absolute bottom-0 left-1/3 w-32 h-32 rounded-full bg-info/5 blur-2xl pointer-events-none" />
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-primary uppercase tracking-widest">Laporan Keuangan</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-bold text-foreground font-display tracking-tight">
              {monthName}
            </h2>
            <p className="text-sm text-muted-foreground mt-0.5">
              {dueThisMonth.length} tagihan jatuh tempo · {fmtShort(totalDueAmount)} cicilan
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        {kpis.map((kpi, i) => {
          ...
          return (
            <div
              key={i}
              className={`laporan-kpi relative overflow-hidden rounded-2xl border ${kpi.border} ${kpi.bg} p-4`}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: kpi.color + '22' }}>
              ...
              <p className="text-xl sm:text-2xl font-bold font-display leading-none" style={{ color: kpi.color }}>
                {kpi.value}
              </p>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-0.5 rounded-full inline-block bg-primary" />Cicilan
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-0.5 rounded-full inline-block bg-success" />Keuntungan
              </span>
            </div>
          </div>
          <div className="h-48 sm:h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyTrend} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                <defs>
                  <linearGradient id="gradCicilan" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={CHART_COLORS.primary} stopOpacity={0.2} />
                    <stop offset="95%" stopColor={CHART_COLORS.primary} stopOpacity={0} />
                  </linearGradient>
                  <linearGradient id="gradUntung" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis tickFormatter={fmtShort} tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" width={40} axisLine={false} tickLine={false} />
                <Tooltip content={<Tooltip_ />} />
                <Area type="monotone" dataKey="cicilan" name="Cicilan" stroke={CHART_COLORS.primary} strokeWidth={2.5} fill="url(#gradCicilan)" dot={{ r: 3, fill: CHART_COLORS.primary, strokeWidth: 0 }} />
                <Area type="monotone" dataKey="keuntungan" name="Keuntungan" stroke={CHART_COLORS.success} strokeWidth={2.5} fill="url(#gradUntung)" dot={{ r: 3, fill: CHART_COLORS.success, strokeWidth: 0 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
        ...
        <div className="laporan-section rounded-2xl border border-border/60 bg-card p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] font-medium px-2 py-1 rounded-full bg-info/10 text-info">Estimasi</span>
          </div>
          <div className="h-44 sm:h-52">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={cashflow} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                <XAxis dataKey="name" tick={{ fontSize: 11 }} stroke="hsl(var(--muted-foreground))" axisLine={false} tickLine={false} />
                <YAxis tickFormatter={fmtShort} tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" width={40} axisLine={false} tickLine={false} />
                <Tooltip content={<Tooltip_ />} />
                <Bar dataKey="masuk" name="Est. Masuk" fill={CHART_COLORS.primary} radius={[6, 6, 0, 0]} maxBarSize={48} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="laporan-section rounded-2xl border border-border/60 bg-card p-4 sm:p-5">
          <div className="mb-4">
            <h3 className="text-sm font-semibold text-foreground">Sisa Terbesar</h3>
            <p className="text-[11px] text-muted-foreground mt-0.5">Top 5 piutang aktif</p>
          </div>
          {topDebtors.length === 0 ? (
                  ...
                  <div key={i}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="w-5 h-5 rounded-md flex items-center justify-center text-[9px] font-bold shrink-0 text-white" style={{ background: colors[i] }}>
                          {i + 1}
                        </span>
                        <span className="text-xs font-medium text-foreground truncate">{d.name}</span>
                      </div>
                      <span className="text-xs font-semibold tabular-nums shrink-0 ml-2" style={{ color: colors[i] }}>
                        {fmtShort(d.sisa)}
                      </span>
                    </div>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              <h3 className="text-sm font-semibold text-foreground">
                {calendarDate
                  ? new Date(calendarDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })
              ...
              (calendarDate ? calendarItems : dueThisMonth).map(t => {
                ...
                return (
                  <button
                    key={t.id}
                    onClick={() => onView(t)}
                    className="w-full text-left flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-muted/50 transition-colors group border border-transparent hover:border-border/50"
                  >
                    <div className={`w-2 h-2 rounded-full shrink-0 ${
                      isUrgent ? 'bg-destructive animate-pulse' :
                      reminder.level === 'warning' ? 'bg-warning' : 'bg-info'
                    }`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate">{t.debitur_nama}</p>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <ChevronRight className="w-3 h-3 text-muted-foreground/50 ml-auto mt-0.5 group-hover:text-foreground transition-colors" />
                    </div>
                  </button>
                );
              })
      ...
      <div className="laporan-section rounded-2xl border border-border/60 bg-card p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          {[
            ...
            },
            {
              ...
              value: fmt(totalDibayar),
              icon: CreditCard,
              color: 'text-success',
              bg: 'bg-success/8',
            },
            {
              label: 'Est. Total Keuntungan',
            ...
            return (
              <div key={i} className={`rounded-xl ${s.bg} p-3 sm:p-4`}>
                ...
                <p className={`text-sm sm:text-base font-bold font-display ${s.color} leading-tight`}>{s.value}</p>
        ...
        <div className="mt-5 pt-4 border-t border-border/50">
          <div className="flex items-center justify-between text-xs mb-2">
            ...
            <span className="font-bold text-foreground">{collectRate.toFixed(1)}%</span>
          </div>
          ...
          <div className="flex justify-between text-[10px] text-muted-foreground mt-1.5">
            ...
            <span>{fmt(totalModal)} target modal</span>
          </div>
        </div>
      </div>
