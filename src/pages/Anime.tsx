...
import { Plus, Search, Tv, ImageIcon, Layers, Filter, X, Star, SlidersHorizontal, Heart, Bookmark } from 'lucide-react';
...
const getNearestDayDistance = (schedule: string) => {
  ...
  for (const d of scheduleDays) {
    ...
    if (dist === 0) dist = 0; // today = 0 distance
...
const Anime = () => {
  ...
  const [stackDetailItem, setStackDetailItem] = useState<AnimeItem | null>(null);
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['anime'] }); setModalOpen(false); setCoverFile(null); setCoverPreview(''); toast({ title: 'Berhasil', description: 'Anime berhasil ditambahkan.' }); },
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['anime'] }); setModalOpen(false); setCoverFile(null); setCoverPreview(''); toast({ title: 'Berhasil', description: 'Anime berhasil diperbarui.' }); },
    onError: (e: any) => { setUploading(false); toast({ title: 'Error', description: e.message, variant: 'destructive' }); },
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => animeService.delete(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['anime'] }); setDeleteOpen(false); toast({ title: 'Berhasil', description: 'Anime berhasil dihapus.' }); },
  ...
  }, [filter, search, animeList, genreFilter, sortMode]);

  const usedGenres = useMemo(() => {
    const genres = new Set<string>();
    animeList.forEach(a => { if (a.genre) a.genre.split(',').map(g => g.trim()).filter(Boolean).forEach(g => genres.add(g)); });
  ...
  const { displayList, stackCounts } = useMemo(() => {
    ...
    standalone.forEach(a => {
      ...
      if (group) {
        const all = [a, ...group].sort((x, y) => (x.season || 1) - (y.season || 1));
  ...
  const filtered = useMemo(() => {
    let result = displayList.filter(a => {
      const matchFilter = filter === 'all' || a.status === filter;
      const matchSearch = a.title.toLowerCase().includes(search.toLowerCase()) || (a.genre || '').toLowerCase().includes(search.toLowerCase());
      const matchGenre = genreFilter === 'all' || (a.genre || '').toLowerCase().includes(genreFilter.toLowerCase());
  ...
  const handleImport = async (items: any[]) => {
    ...
    toast({ title: 'Import Berhasil', description: `${items.length} anime berhasil diimpor.` });
  };
  ...
  const stackItems = useMemo(() => {
    ...
    return animeList
      .filter(a => a.title.trim().toLowerCase() === stackViewTitle || (a.parent_title || '').trim().toLowerCase() === stackViewTitle)
    ...
    <div ref={containerRef}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="page-subtitle">Kelola koleksi anime yang sedang, sudah, atau akan ditonton.</p>
        </div>
      ...
      <div className="flex flex-col gap-3 mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari judul atau genre anime..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-primary transition-all" />
          </div>
        ...
        <div className="flex gap-2 flex-wrap overflow-x-auto pb-1">
          {(['all', 'on-going', 'completed', 'planned'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all whitespace-nowrap min-h-[40px] ${filter === f ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-accent'}`}>
              {f === 'all' ? `Semua (${animeList.length})` : `${statusLabel(f)} (${animeList.filter(a => a.status === f).length})`}
            </button>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-muted-foreground">Belum ada anime yang tercatat.</p>
          <button onClick={openAdd} className="mt-3 text-sm text-primary font-medium hover:underline">+ Tambah anime pertama</button>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
          {filtered.map(anime => (
            <MediaCard
              ...
              season={anime.season}
              cour={anime.cour}
              streamingUrl={anime.streaming_url}
              schedule={anime.schedule}
              synopsis={anime.synopsis}
              notes={anime.notes}
              stackCount={stackCounts[anime.id] || 0}
              type="anime"
              onClick={stackCounts[anime.id] > 0 ? () => openStackView(anime.parent_title || anime.title) : undefined}
              onEdit={() => stackCounts[anime.id] > 0 ? openStackView(anime.parent_title || anime.title) : openEdit(anime)}
              onDelete={() => stackCounts[anime.id] > 0 ? openStackView(anime.parent_title || anime.title) : (() => { setDeleteItem(anime); setDeleteOpen(true); })()}
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display">{editItem ? 'Edit Anime' : 'Tambah Anime Baru'}</DialogTitle>
            <DialogDescription>{editItem ? 'Perbarui informasi anime.' : 'Isi detail anime yang ingin dicatat.'}</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div>
              <label className="label-text mb-1.5 block">Judul Anime *</label>
              <input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="cth: Solo Leveling" className={inputClass} required />
            </div>
            ...
            {/* Parent title — searchable combo input */}
            <div className="relative">
              <label className="label-text mb-1.5 block">Kelompokkan dengan Anime Lain</label>
              <input
                type="text"
                value={parentSearch}
              ...
              <p className="helper-text mt-1">Ketik untuk mencari atau pilih judul anime yang sama untuk menumpuk card</p>
            </div>
            ...
            <div>
              ...
              <textarea value={form.synopsis} onChange={e => setForm({ ...form, synopsis: e.target.value })} placeholder="Ringkasan cerita anime..." rows={2} className={`${inputClass} resize-none`} />
            </div>
            <div>
              ...
              <div key={item.id} className="rounded-xl border border-border overflow-hidden">
                <button onClick={() => { setStackViewOpen(false); setTimeout(() => setStackDetailItem(item), 150); }}
                  ...
                  <div className="w-10 h-14 rounded-md bg-muted overflow-hidden shrink-0">
                    {item.cover_url ? <img src={item.cover_url} alt={item.title} className="w-full h-full object-cover" /> : <Tv className="w-4 h-4 text-muted-foreground/30 m-auto mt-4" />}
                  </div>
                ...
                <div className="flex border-t border-border divide-x divide-border">
                  <button onClick={() => { setStackViewOpen(false); setTimeout(() => setStackDetailItem(item), 150); }}
                    ...
                    <Star className="w-3.5 h-3.5" /> Detail
                  </button>
        ...
        <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
          {stackDetailItem && (() => {
            ...
            const genreColorMap: Record<string, string> = {
              ...
              'Sci-Fi': 'bg-info/15 text-info', 'Isekai': 'bg-success/15 text-success',
            };
            return (
              <>
                <DialogHeader>
