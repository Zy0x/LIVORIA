...
interface DonghuaDeleteDialogProps {
  open: boolean;
  deleteItem: DonghuaItem | null;
  batchIds: string[];
  deleting: boolean;
...
export function DonghuaDeleteDialog({
  open,
  deleteItem,
  batchIds,
  deleting,
        ...
        <div className="bg-destructive/10 p-6 flex flex-col items-center text-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-foreground font-display">
              {batchIds.length > 0 ? 'Hapus Batch' : `Hapus ${deleteItem?.is_movie ? 'Film' : 'Donghua'}`}
            </h3>
